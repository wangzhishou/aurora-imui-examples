...
class LightVoiceViewHolder<MESSAGE : IMessage>(itemView: View, private var mIsSender: Boolean) : BaseMessageViewHolder<MESSAGE>(itemView), MsgListAdapter.DefaultMessageViewHolder {
    ...
    override fun onBind(message: MESSAGE) {
        ...
        mMediaPlayer.setOnErrorListener { mp, _, extra -> false }
        if (message.timeString != null) {
            mDateTv!!.text = message.timeString
        }
        val isAvatarExists =
            message.fromUser.avatarFilePath != null && !message.fromUser.avatarFilePath.isEmpty()
        ...
        val lengthStr =
            duration.toString() + mContext.getString(cn.jiguang.imui.R.string.aurora_symbol_second)
        val width =
            (-0.04 * duration.toDouble() * duration.toDouble() + 4.526 * duration + 75.214).toInt()
        ...
        mMsgTv!!.setOnClickListener(View.OnClickListener {
            ...
            Log.e(
                "VoiceViewHolder",
                "MediaPlayer playing " + mMediaPlayer.isPlaying + "now position " + adapterPosition
            )
